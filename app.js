...
const DBN="tcesp-irapua-v6",STORE="dados",KEY="base";
...
function tceExpenseUrl(year){return `https://transparencia.tce.sp.gov.br/municipio/irapua/${year}/despesas`}
...
async function loadPermanentBaseline(force=false){
  ...
  let base={_baselineVersion:manifest.version||cfg.version,despesas:[],receitas:[],exercicios:[...(manifest.exercicios||[])],apiUpdates:manifest.apiUpdates||{},autoCheck:manifest.autoCheck||{}};
  for(let i=0;i<manifest.arquivos.length;i++){
    ...
    base.despesas.push(...pack.despesas);
    base.receitas.push(...pack.receitas);
  }
  base.exercicios=[...new Set([...base.exercicios,...base.despesas.map(x=>+x.ano),...base.receitas.map(x=>+x.ano)])].filter(Boolean).sort((a,b)=>a-b);
  base._baselineTotals={despesas:base.despesas.length,receitas:base.receitas.length};
  base._baselineCoverageWarnings=manifest.coverageWarnings||[];
  return base
}
...
(async()=>{
  ...
  try{
    ...
    if(!hasUsableData(db)){
      let old=null;
      try{
        let raw=localStorage.getItem("tcesp-irapua-data")||localStorage.getItem("tcesp-data");
    ...
    try{
      await save();
      localStorage.removeItem("tcesp-irapua-data");
    }catch(e){
      initError=e;
